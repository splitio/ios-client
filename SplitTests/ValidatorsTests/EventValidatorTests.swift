...
class EventValidatorTests: XCTestCase {
    ...
    func testNullKey() {
        let errorInfo = validator.validate(
            key: nil,
            trafficTypeName: "custom",
            eventTypeId: "type1",
            value: nil,
    ...
    func testNullType() {
        ...
        XCTAssertTrue(errorInfo?.isError ?? false)
        XCTAssertEqual(
            "you passed a null or undefined event_type, event_type must be a non-empty String",
            errorInfo?.errorMessage)
        XCTAssertEqual(errorInfo?.warnings.count, 0)
    }
    ...
    func testTypeName() {
        ...
        let errorInfo1 = validator.validate(
            key: "key1",
            trafficTypeName: "custom",
            eventTypeId: nameHelper.validAllValidChars,
            value: nil,
            properties: nil,
            isSdkReady: true)
        let errorInfo2 = validator.validate(
            key: "key1",
            trafficTypeName: "custom",
            eventTypeId: nameHelper.validStartNumber,
            value: nil,
            properties: nil,
            isSdkReady: true)
        let errorInfo3 = validator.validate(
            key: "key1",
            trafficTypeName: "custom",
            eventTypeId: nameHelper.invalidChars,
            value: nil,
            properties: nil,
            isSdkReady: true)
        let errorInfo4 = validator.validate(
            key: "key1",
            trafficTypeName: "custom",
            eventTypeId: nameHelper.invalidUndercoreStart,
            value: nil,
            properties: nil,
            isSdkReady: true)
        let errorInfo5 = validator.validate(
            key: "key1",
            trafficTypeName: "custom",
            eventTypeId: nameHelper.invalidHypenStart,
            value: nil,
            properties: nil,
            isSdkReady: true)
    ...
    func testNullTrafficType() {
        ...
        XCTAssertTrue(errorInfo?.isError ?? false)
        XCTAssertEqual(
            "you passed a null or undefined traffic_type_name, traffic_type_name must be a non-empty string",
            errorInfo?.errorMessage)
        XCTAssertEqual(errorInfo?.warnings.count, 0)
    }
    ...
    func testEmptyTrafficType() {
        ...
        XCTAssertTrue(errorInfo?.isError ?? false)
        XCTAssertEqual(
            "you passed an empty traffic_type_name, traffic_type_name must be a non-empty string",
            errorInfo?.errorMessage)
        XCTAssertEqual(errorInfo?.warnings.count, 0)
    }
    ...
    func testNoChachedServerTrafficType() {
        ...
        XCTAssertEqual(
            "traffic_type_name nocached does not have any corresponding feature flags in this environment, make sure you’re tracking your events to a valid traffic type defined in the Split user interface",
    ...
    func testNoChachedServerAndUppercasedTrafficType() {
        ...
        XCTAssertEqual(
            ...
            errorInfo?.warnings[.trafficTypeNameHasUppercaseChars])
        XCTAssertEqual(
            "traffic_type_name noCached does not have any corresponding feature flags in this environment, make sure you’re tracking your events to a valid traffic type defined in the Split user interface",
            errorInfo?.warnings[.trafficTypeWithoutSplitInEnvironment])
    }
