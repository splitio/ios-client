...
enum TestingHelper {
    ...
    static func createImpressionsCount(count: Int = 10, randomId: Bool = true) -> [ImpressionsCountPerFeature] {
        ...
        for i in 0 ..< count {
            var count = ImpressionsCountPerFeature(
                feature: "feature\(i)",
                timeframe: Date().unixTimestampInMiliseconds(),
                count: 1)
            if randomId {
    ...
    static func createTelemetryConfig() -> TelemetryConfig {
        return TelemetryConfig(
            ...
            rates: nil,
            urlOverrides: nil,
            impressionsQueueSize: 9,
            eventsQueueSize: 9,
            impressionsMode: 9,
            impressionsListenerEnabled: true,
            httpProxyDetected: true,
            activeFactories: 1,
            redundantFactories: 12,
            timeUntilReady: 9,
    ...
    static func createTelemetryStats() -> TelemetryStats {
        return TelemetryStats(
            ...
            methodLatencies: nil,
            methodExceptions: nil,
            httpErrors: nil,
            httpLatencies: nil,
            tokenRefreshes: 1,
            authRejections: 1,
            impressionsQueued: 1,
            impressionsDeduped: 1,
            impressionsDropped: 1,
            splitCount: 1,
            segmentCount: 1,
            segmentKeyCount: 2,
            sessionLengthMs: 88888,
            eventsQueued: 1,
            eventsDropped: 1,
            streamingEvents: nil,
            tags: nil,
            updatesFromSse: TelemetryUpdatesFromSse(splits: 10, mySegments: 20))
    }
    ...
    static func buildSegmentsChange(
        count: Int64 = 5,
        ...
        for _ in 0 ..< count - 1 {
            msC += msSum
            mlsC += mlsSum
            res.append(newAllSegmentsChange(
                ms: msSeg,
                msCn: msC,
                mls: mlsSeg,
                mlsCn: mlsC))
    ...
    static func newAllSegmentsChange(
        ms: [String] = ["s1", "s1"],
        msCn: Int64 = -1,
        mls: [String] = ["ls1", "ls2"],
        mlsCn: Int64 = -1) -> AllSegmentsChange {
    ...
    static func segmentsSyncResult(
        _ result: Bool = true,
        msCn: Int64 = 300,
        mlsCn: Int64 = 400,
        msUpd: Bool = true,
        mlsUpd: Bool = true) -> SegmentsSyncResult {
        return SegmentsSyncResult(
            success: result,
            msChangeNumber: msCn,
            mlsChangeNumber: mlsCn,
            msUpdated: msUpd,
            mlsUpdated: mlsUpd)
    }

    static func newAllSegmentsChangeJson(
        ms: [String] = ["s1", "s1"],
        msCn: Int64? = nil,
        mls: [String] = ["ls1", "ls2"],
        mlsCn: Int64? = nil) -> String {
    ...
    static func createRuleBasedSegment(
        name: String = "test_rbs",
        trafficTypeName: String = "user",
        changeNumber: Int64 = 1000,
        status: Status = .active,
        conditions: [Condition]? = nil,
        excluded: Excluded? = nil) -> RuleBasedSegment {
