...
class EvaluatorTests: XCTestCase {
    ...
    func testsTrafficAllocation50DefaultRule50() {
        ...
        do {
            // Changing key
            treatmentOn = try evaluator.evalTreatment(
                matchingKey: theKey,
                bucketingKey: nil,
                splitName: split.name!,
                attributes: nil)
            treatmentOff = try evaluator.evalTreatment(
                matchingKey: "aa9055eb-710c-4817-93bc-6906db5f4934",
                bucketingKey: nil,
                splitName: splitName,
                attributes: nil)
            treatmentOutOfSplit = try evaluator.evalTreatment(
            ...
            treatmentSeedDefaultOff = try evaluator.evalTreatment(
                matchingKey: theKey,
                bucketingKey: nil,
                splitName: splitName,
                attributes: nil)
            ...
            treatmentSeedOutOfSplit = try evaluator.evalTreatment(
                matchingKey: theKey,
                bucketingKey: nil,
                splitName: splitName,
                attributes: nil)
    ...
    func inLargeSegmentWhiteListTest(key: String, treatment: String = "on") {
        ...
        let evaluator = customEvaluator(
            splitFile: "in_large_segment_whitelist_split",
            largeSegments: ["segment1"]) as! DefaultEvaluator
        evaluator.splitter = SplitterAllocationFake()
