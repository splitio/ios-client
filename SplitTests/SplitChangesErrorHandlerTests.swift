...
class SplitChangesErrorHandlerTests: XCTestCase {
    ...
    func testSplitChangesWithOutdatedProxyError() {
        ...
        clientWithOverriddenEndpoint.getSplitChanges(
            since: 1000,
            rbSince: 500,
            till: nil,
            headers: nil,
            spec: testSpec) { dataResult in
                do {
        ...
        if case let .outdatedProxyError(code, spec)? = outdatedProxyError {
    ...
    func testSplitChangesWithDifferentStatusCode() {
        ...
        if case let .unknown(code, _)? = httpError {
    ...
    func testSplitChangesWithDifferentSpec() {
        ...
        if case let .clientRelated(code, _)? = httpError {
            XCTAssertEqual(code, HttpCode.badRequest)
        } else {
