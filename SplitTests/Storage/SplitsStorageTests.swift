...
class SplitsStorageTest: XCTestCase {
    ...
    func testUpdateSplits() {
        ...
        let processedChange = ProcessedSplitChange(
            activeSplits: [newSplit(name: "added"), newSplit(name: "added1")],
        ...
        XCTAssertEqual(0, splits.keys.filter { $0 == "s1" }.count)
        XCTAssertEqual(1, splits.keys.filter { $0 == "added" }.count)
        XCTAssertEqual(1, splits.keys.filter { $0 == "added1" }.count)
    ...
    func testGetMany() {
        ...
        XCTAssertEqual(1, splits.keys.filter { $0 == "s1" }.count)
        XCTAssertEqual(1, splits.keys.filter { $0 == "s9" }.count)
    }
    ...
    func testUpdateResult() {
        ...
        var processedChange = ProcessedSplitChange(
            activeSplits: [],
            archivedSplits: [newSplit(
                name: "s1",
                status: .archived,
                sets: ["set1"])],
            changeNumber: 999,
        ...
        processedChange = ProcessedSplitChange(
            activeSplits: [newSplit(name: "s1", status: .archived, sets: ["set1"])],
            archivedSplits: [],
            changeNumber: 9999,
            updateTimestamp: 8888)
    ...
    private func getTestSnapshot(count: Int = 10, sets: [[String]]? = nil) -> SplitsSnapshot {
        ...
        return SplitsSnapshot(
            changeNumber: dummyChangeNumber,
            splits: splits,
    ...
    private func unsupportedMatcherSplit() -> Split {
        return Split(
            ...
            status: Status.active,
            sets: [],
            json: SplitTestHelper.getUnsupportedMatcherSplitJson(sourceClass: self)!)
    }
}
