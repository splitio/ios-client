...
class PersistentSplitsStorageTest: XCTestCase {
    ...
    func testUpdateProcessedChange() {
        let activeSplits = [
            newSplit(name: "ac1", trafficType: "t1"),
            newSplit(name: "ac2", trafficType: "t1"),
            newSplit(name: "ac3", trafficType: "t1"),
        ]
        let archivedSplits = [
            newSplit(name: "ar1", trafficType: "t2", status: .archived),
            newSplit(name: "ar2", trafficType: "t2", status: .archived),
        ]
    ...
    func testUpdateBySetFilter() {
        ...
        let filter = try? Json.decodeFrom(
            json: generalInfoDao.stringValue(info: .bySetsFilter) ?? "",
            to: SplitFilter.self)
