...
class ImpressionsCounterTest: XCTestCase {
    ...
    func testTruncateTime() {
        let time1 = Date.truncateTimeframe(millis: makeTimestamp(hour: 10, minute: 53, second: 12).unixTimestampInMiliseconds())
        let timeframe1 = makeTimestamp(hour: 10, minute: 0, second: 0).unixTimestampInMiliseconds()

        let time2 = Date.truncateTimeframe(millis: makeTimestamp(hour: 10, minute: 0, second: 0).unixTimestampInMiliseconds())
        let timeframe2 = makeTimestamp(hour: 10, minute: 0, second: 0).unixTimestampInMiliseconds()

        let time3 = Date.truncateTimeframe(millis: makeTimestamp(hour: 10, minute: 53, second: 0).unixTimestampInMiliseconds())
        let timeframe3 = makeTimestamp(hour: 10, minute: 0, second: 0).unixTimestampInMiliseconds()

        let time4 = Date.truncateTimeframe(millis: makeTimestamp(hour: 10, minute: 0, second: 12).unixTimestampInMiliseconds())
        let timeframe4 = makeTimestamp(hour: 10, minute: 0, second: 0).unixTimestampInMiliseconds()

        let time2019 = Date.truncateTimeframe(millis: makeTimestamp(year: 2019, month: 6, hour: 1, minute: 18, second: 12).unixTimestampInMiliseconds())
        let timeframe2019 = makeTimestamp(year: 2019, month: 6, hour: 1, minute: 0, second: 0).unixTimestampInMiliseconds()

        let time2021 = Date.truncateTimeframe(millis: makeTimestamp(year: 2021, month: 12, hour: 23, minute: 59, second: 50).unixTimestampInMiliseconds())
        let timeframe2021 = makeTimestamp(year: 2021, month: 12, hour: 23, minute: 0, second: 0).unixTimestampInMiliseconds()

        let time1970 = Date.truncateTimeframe(millis: makeTimestamp(year: 1970, hour: 1, minute: 18, second: 12).unixTimestampInMiliseconds())
    ...
    func testBasicUsage() {
        ...
        let nextHourTimestamp = makeTimestamp(year: 2020, month: 9, day: 2, hour: 11, minute: 10, second: 12).unixTimestampInMiliseconds()
        let truncatedNextHourTimestamp = makeTimestamp(year: 2020, month: 9, day: 2, hour: 11, minute: 0, second: 0).unixTimestampInMiliseconds()
        counter.inc(featureName: "feature1", timeframe: timestamp, amount: 1)
        counter.inc(featureName: "feature1", timeframe: timestamp + 1, amount: 1)
        counter.inc(featureName: "feature1", timeframe: timestamp + 2, amount: 1)
        counter.inc(featureName: "feature2", timeframe: timestamp + 3, amount: 2)
        ...
        XCTAssertEqual(counted1[ImpressionsCounter.Key(featureName: "feature1", timeframe: truncatedNextHourTimestamp)], 3)
        XCTAssertEqual(counted1[ImpressionsCounter.Key(featureName: "feature2", timeframe: truncatedNextHourTimestamp)], 4)
        XCTAssertEqual(counter.popAll().count, 0)
    }
    ...
    func testConcurrency() throws {
        ...
        XCTAssertEqual(counted[ImpressionsCounter.Key(featureName: "feature1", timeframe: truncatedTimestamp)], expectedValue)
        XCTAssertEqual(counted[ImpressionsCounter.Key(featureName: "feature2", timeframe: truncatedTimestamp)], expectedValue)
        XCTAssertEqual(counted[ImpressionsCounter.Key(featureName: "feature1", timeframe: truncatedNextHourTimestamp)], expectedValue)
        XCTAssertEqual(counted[ImpressionsCounter.Key(featureName: "feature2", timeframe: truncatedNextHourTimestamp)], expectedValue)
        XCTAssertEqual(counter.popAll().count, 0)
    ...
    private func makeTimestamp(year: Int = 2020, month: Int = 1, day: Int = 1, hour: Int = 1, minute: Int = 1, second: Int = 0) -> Date {
        let calendar = Calendar.current
        let date = DateComponents(calendar: calendar, timeZone: TimeZone.current, era: nil, year: year, month: month, day: day, hour: hour, minute: minute, second: second, nanosecond: nil, weekday: nil, weekdayOrdinal: nil, quarter: nil, weekOfMonth: nil, weekOfYear: nil, yearForWeekOfYear: nil).date
        return date!
    }
