...
class SimpleTunnelEstablisherTests: XCTestCase {
    ...
    func testEstablishTunnel_cancelledConnection_returns999Error() {
        // Given: a valid ProxyConfiguration
        let proxyUrl = URL(string: "https://proxy.example.com:8080")!
        let caCertData = Data([0x01, 0x02, 0x03])
        let proxyConfig = try! ProxyConfiguration(proxyURL: proxyUrl, caCertificateData: caCertData)
        let establisher = SimpleTunnelEstablisher()
        let expectation = self.expectation(description: "Tunnel establishment cancelled")
        
        // When: simulate a cancelled connection (NSURLErrorCancelled = -999)
        establisher.establishTunnel(to: "cancelled.example.com", port: 443, through: proxyConfig) { streamTask, error in
            // Then: should return -999 error
            XCTAssertNil(streamTask)
            XCTAssertNotNil(error)
            if let nsError = error as NSError? {
                XCTAssertEqual(nsError.domain, "SimpleTunnelEstablisher")
                XCTAssertEqual(nsError.code, -999)
                XCTAssertTrue(nsError.localizedDescription.contains("cancelled"))
            }
            expectation.fulfill()
        }
        waitForExpectations(timeout: 2.0, handler: nil)
