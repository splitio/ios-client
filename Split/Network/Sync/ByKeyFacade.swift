...
class DefaultByKeyFacade: ByKeyFacade {
    ...
    func loadAttributesFromCache(forKey key: String) {
        doInAll(forMatchingKey: key) { group in
            ...
            group.eventsManager.notifyEvent(.attributesLoadedFromCache, SplitMetadata(key: "Message", value: "Attributes from cache ready"))
        }
    ...
    func notifyMySegmentsUpdated(forKey key: String) {
        doInAll(forMatchingKey: key) { group in
            group.eventsManager.notifyEvent(.mySegmentsUpdated, SplitMetadata(key: "Message", value: "Segments updated for key \(key)"))
        }
    }

    func notifyMyLargeSegmentsUpdated(forKey key: String) {
        doInAll(forMatchingKey: key) { group in
            group.eventsManager.notifyEvent(.myLargeSegmentsUpdated,SplitMetadata(key: "Message", value: "Large segments updated  for key \(key)"))
        }
    }
