...
class DefaultRestClient: SplitApiRestClient {
    ...
    init(
        httpClient: HttpClient = RestClientConfiguration.httpClient,
    ...
    func execute<T>(
                        ...
                        do {
                            if let customDecoder = customDecoder {
                                ...
                                if let parsedObject = try json.decodeWith(customDecoder) {
                                    completion(DataResult { parsedObject })
                                } else {
                                    completion(DataResult { nil })
                                }
                            } else {
                        ...
                        completion(DataResult {
                            ...
                            if response.code >= HttpCode.badRequest, response.code < HttpCode.internalServerError {
                                throw HttpError.clientRelated(
                                    code: response.code,
                                    internalCode: InternalHttpErrorCode.noCode)
                            }
        ...
        } catch let HttpError.couldNotCreateRequest(message) {
...
extension DefaultRestClient: RestClient {
    func isServerAvailable(_ url: URL) -> Bool {
        return isServerAvailable(path: url.absoluteString)
    }
    ...
    func isEventsServerAvailable() -> Bool {
        return isServerAvailable(endpointFactory.serviceEndpoints.eventsEndpoint)
    }

    func isSdkServerAvailable() -> Bool {
        return isServerAvailable(endpointFactory.serviceEndpoints.sdkEndpoint)
    }
}
