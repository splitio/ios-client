...
class Condition: NSObject, Codable {
    ...
    required init(from decoder: Decoder) throws {
        if let values = try? decoder.container(keyedBy: CodingKeys.self) {
            self.conditionType = try? values.decode(ConditionType.self, forKey: .conditionType)
            self.matcherGroup = try? values.decode(MatcherGroup.self, forKey: .matcherGroup)
            self.partitions = try? values.decode([Partition].self, forKey: .partitions)
            self.label = (try? values.decode(String.self, forKey: .label)) ?? ""
        }
    }

    func match(values: EvalValues, context: EvalContext?) throws -> Bool {
        if let matcherG = matcherGroup, let matchers = matcherG.matchers {
            var results: [Bool] = []
                ...
                if matcherEvaluator.getMatcherType() != MatcherType.dependency &&
                    ...
                    if !matcherEvaluator.matcherHasAttribute() {
                        ...
                        if values.attributes == nil || values.attributes![att] == nil {
                            ...
                            let newValues = EvalValues(
                                matchValue: values.attributes?[att] ?? "null",
                                matchingKey: values.matchingKey,
                                bucketingKey: nil,
                                attributes: nil)
                            result = matcherEvaluator.evaluate(values: newValues, context: context)
