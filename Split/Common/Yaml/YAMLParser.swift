...
private func parseFlowSeq(_ acc: [Yaml]) -> (Context) -> YAMLResult<ContextValue> {
    return { context in
        ...
        let cv = Resulter.lift(context)
            >>- ignoreSpace
            >>=- (acc.isEmpty ? Resulter.lift : expect(Yaml.TokenType.comma, message: "expected comma"))
            >>- ignoreSpace
            >>=- parse
        let v = cv >>- getValue
        let c = cv
...
private func parseliteral(_ context: Context) -> YAMLResult<ContextValue> {
    ...
    let c = Resulter.guard(
        error(error0)(context),
