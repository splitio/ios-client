            ...
            for tokenPattern in tokenPatterns {
                ...
                if range.location != NSNotFound {
                    ...
                    case .folded, .literal:
                        ...
                        let blockPattern = Yaml.Regex.regex(
                            "^(\(bBreak) *)*(\(bBreak)" +
                        ...
                        let block = (
                            lead
                                ...
                                |> Yaml.Regex.replace(
                                    Yaml.Regex.regex("\(bBreak) {0,\(lastindent)}"),
                                    template: "\n")) +
                            (
                                Yaml.Regex.matches(text, regex: Yaml.Regex.regex("^\(bBreak)")) && lead.endIndex > lead
                                    .startIndex
                                    ? "\n" : "")
