internal enum YAMLResult<T> {
    ...
    public func map<U>(f: (T) -> U) -> YAMLResult<U> {
        ...
        case let .value(v): return .value(f(v))
        }
    }
...
func <*> <T, U>(f: YAMLResult<(T) -> U>, x: YAMLResult<T>) -> YAMLResult<U> {
    switch (x, f) {
    case let (.error(e), _): return .error(e)
    case let (.value, .error(e)): return .error(e)
    case let (.value(x), .value(f)): return .value(f(x))
    }
}
...
extension Yaml {
    ...
    static func `guard`(_ error: @autoclosure () -> String, check: Bool) -> YAMLResult<Void> {
