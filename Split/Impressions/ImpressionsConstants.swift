//
//  ImpressionsConstants.swift
//  Split
//
//  Created by Natalia  Stele on 03/01/2018.
//

import Foundation

public class ImpressionsConstants {
    
    public static let NO_CONDITION_MATCHED: String = "default rule"
    public static let KILLED: String = "killed"
    public static let SPLIT_NOT_FOUND: String = "definition not found"
    public static let NOT_IN_SPLIT: String = "not in split"
    public static let MATCHER_NOT_FOUND: String = "matcher not found"
    public static let EXCEPTION: String = "exception"
    
    public static let SPLIT_SDK_VERSION: String = "ios-0.1.0"
    public static let IMPRESSIONS_MACHINE_IP: String = "123.123.123.123"

}



